		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(
						"huaweicloudstack_blockstorage_volume_v2.volume_1", "name", "volume_1"),
					resource.TestCheckResourceAttr(
						"huaweicloudstack_blockstorage_volume_v2.volume_1", "size", "1"),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckBlockStorageV2VolumeMetadata(&volume, "foo", "bar"),
					resource.TestCheckResourceAttr(
						"huaweicloudstack_blockstorage_volume_v2.volume_1", "name", "volume_1-updated"),
					resource.TestCheckResourceAttr(
						"huaweicloudstack_blockstorage_volume_v2.volume_1", "size", "2"),
				),
			},
		},
	})
...
func testAccCheckBlockStorageV2VolumeDestroy(s *terraform.State) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating HuaweiCloudStack block storage client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "huaweicloudstack_blockstorage_volume_v2" {
			continue
		}
...
resource "huaweicloudstack_compute_volume_attach_v2" "va_1" {
  instance_id = "${huaweicloudstack_compute_instance_v2.basic.id}"
