		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeV2InterfaceAttachExists("huaweicloudstack_compute_interface_attach_v2.ai_1", &ai),
				),
			},
		},
	})
...
func testAccCheckComputeV2InterfaceAttachDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "huaweicloudstack_compute_interface_attach_v2" {
			continue
		}
