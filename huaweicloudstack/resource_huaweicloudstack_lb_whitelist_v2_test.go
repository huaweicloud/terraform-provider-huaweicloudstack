		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLBV2WhitelistExists("huaweicloudstack_lb_whitelist_v2.whitelist_1", &whitelist),
				),
			},
			{
				Config: TestAccLBV2WhitelistConfig_update,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("huaweicloudstack_lb_whitelist_v2.whitelist_1", "enable_whitelist", "true"),
				),
			},
		},
	})
...
func testAccCheckLBV2WhitelistDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "huaweicloudstack_lb_whitelist_v2" {
			continue
		}
