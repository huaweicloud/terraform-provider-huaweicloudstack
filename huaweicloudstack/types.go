...
func (lrt *LogRoundTripper) RoundTrip(request *http.Request) (*http.Response, error) {
	...
	if lrt.OsDebug {
		log.Printf("[DEBUG] HuaweiCloudStack Request URL: %s %s", request.Method, request.URL)
		log.Printf("[DEBUG] HuaweiCloudStack Request Headers:\n%s", FormatHeaders(request.Header, "\n"))
	...
	if lrt.OsDebug {
		log.Printf("[DEBUG] HuaweiCloudStack Response Code: %d", response.StatusCode)
		log.Printf("[DEBUG] HuaweiCloudStack Response Headers:\n%s", FormatHeaders(response.Header, "\n"))
...
func (lrt *LogRoundTripper) logRequest(original io.ReadCloser, contentType string) (io.ReadCloser, error) {
	...
	if strings.HasPrefix(contentType, "application/json") {
		...
		log.Printf("[DEBUG] HuaweiCloudStack Request Body: %s", debugInfo)
	} else {
		log.Printf("[DEBUG] HuaweiCloudStack Request Body: %s", bs.String())
	}
...
func (lrt *LogRoundTripper) logResponse(original io.ReadCloser, contentType string) (io.ReadCloser, error) {
	if strings.HasPrefix(contentType, "application/json") {
		...
		debugInfo := lrt.formatJSON(bs.Bytes())
		if debugInfo != "" {
			log.Printf("[DEBUG] HuaweiCloudStack Response Body: %s", debugInfo)
		}
		return ioutil.NopCloser(strings.NewReader(bs.String())), nil
	}

	log.Printf("[DEBUG] Not logging because HuaweiCloudStack response body isn't JSON")
	return original, nil
}
...
func (lrt *LogRoundTripper) formatJSON(raw []byte) string {
	...
	if err != nil {
		log.Printf("[DEBUG] Unable to parse HuaweiCloudStack JSON: %s", err)
		return string(raw)
	}
	...
	if err != nil {
		log.Printf("[DEBUG] Unable to re-marshal HuaweiCloudStack JSON: %s", err)
		return string(raw)
	}
