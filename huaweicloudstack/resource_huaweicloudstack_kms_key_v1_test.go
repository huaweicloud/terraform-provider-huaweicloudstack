		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckKmsV1KeyExists("huaweicloudstack_kms_key_v1.key_2", &key),
					resource.TestCheckResourceAttr(
						"huaweicloudstack_kms_key_v1.key_2", "key_alias", keyAlias),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckKmsV1KeyExists("huaweicloudstack_kms_key_v1.key_2", &key),
					resource.TestCheckResourceAttr(
						"huaweicloudstack_kms_key_v1.key_2", "key_alias", keyAliasUpdate),
					resource.TestCheckResourceAttr(
						"huaweicloudstack_kms_key_v1.key_2", "key_description", "key update description"),
				),
			},
		},
	})
...
func testAccCheckKmsV1KeyDestroy(s *terraform.State) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating HuaweiCloudStack kms client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "huaweicloudstack_kms_key_v1" {
			continue
		}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckKmsV1KeyExists("huaweicloudstack_kms_key_v1.bar", &key1),
					resource.TestCheckResourceAttr("huaweicloudstack_kms_key_v1.bar", "is_enabled", "true"),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckKmsV1KeyExists("huaweicloudstack_kms_key_v1.bar", &key2),
					resource.TestCheckResourceAttr("huaweicloudstack_kms_key_v1.bar", "is_enabled", "false"),
			...
			},
			{
				Config: testAccKmsKey_enabled(rName),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckKmsV1KeyExists("huaweicloudstack_kms_key_v1.bar", &key3),
					resource.TestCheckResourceAttr("huaweicloudstack_kms_key_v1.bar", "is_enabled", "true"),
...
func testAccKmsV1Key_basic(keyAlias string) string {
	return fmt.Sprintf(`
		resource "huaweicloudstack_kms_key_v1" "key_2" {
			key_alias = "%s"
			pending_days = "7"
		}
	`, keyAlias)
...
func testAccKmsKey_enabled(rName string) string {
	return fmt.Sprintf(`
resource "huaweicloudstack_kms_key_v1" "bar" {
