...
func computeInterfaceAttachV2DetachFunc(
	computeClient *golangsdk.ServiceClient, instanceId, attachmentId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		log.Printf("[DEBUG] Attempting to detach huaweicloudstack_compute_interface_attach_v2 %s from instance %s",
		...
		log.Printf("[DEBUG] huaweicloudstack_compute_interface_attach_v2 %s is still active.", attachmentId)
		return nil, "", nil
	}
}
...
func computeInterfaceAttachV2ParseID(id string) (string, string, error) {
	...
	if len(idParts) < 2 {
		return "", "", fmt.Errorf("Unable to determine huaweicloudstack_compute_interface_attach_v2 %s ID", id)
	}
