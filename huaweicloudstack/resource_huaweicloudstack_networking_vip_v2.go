...
func resourceNetworkingVIPV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] Waiting for HuaweiCloudStack Neutron VIP (%s) to become available.", vip.ID)
...
func waitForNetworkVIPDelete(networkingClient *golangsdk.ServiceClient, vipid string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		log.Printf("[DEBUG] Attempting to delete HuaweiCloudStack Neutron VIP %s", vipid)

		p, err := ports.Get(networkingClient, vipid).Extract()
		if err != nil {
			if _, ok := err.(golangsdk.ErrDefault404); ok {
				log.Printf("[DEBUG] Successfully deleted HuaweiCloudStack VIP %s", vipid)
		...
		log.Printf("[DEBUG] HuaweiCloudStack VIP %s still active.\n", vipid)
		return p, "ACTIVE", nil
	}
}
