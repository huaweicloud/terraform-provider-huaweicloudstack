...
func resourceASGroupCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if len(tagRaw) > 0 {
		...
		if tagErr := tags.Update(asClient, asgId, taglist).ExtractErr(); tagErr != nil {
...
func resourceASGroupRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error fetching HuaweiCloudStack ASGroup tags: %s", err)
	}
	...
	if err := d.Set("tags", tagmap); err != nil {
		return fmt.Errorf("Error saving tags for HuaweiCloudStack ASGroup (%s): %s", d.Id(), err)
	}
