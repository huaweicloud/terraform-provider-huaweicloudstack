		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkingV2FloatingIPExists(
						"huaweicloudstack_networking_floatingip_associate_v2.fip_1", &fip),
					resource.TestCheckResourceAttrPtr(
						"huaweicloudstack_networking_floatingip_associate_v2.fip_1", "floating_ip", &fip.FloatingIP),
					resource.TestCheckResourceAttrPtr(
						"huaweicloudstack_networking_floatingip_associate_v2.fip_1", "port_id", &fip.PortID),
				),
			},
		},
	})
...
func testAccCheckNetworkingV2FloatingIPAssociateDestroy(s *terraform.State) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating HuaweiCloudStack floating IP: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "huaweicloudstack_networking_floatingip_v2" {
			continue
		}
...
resource "huaweicloudstack_networking_port_v2" "port_1" {
  admin_state_up = "true"
  network_id = "${huaweicloudstack_networking_subnet_v2.subnet_1.network_id}"
