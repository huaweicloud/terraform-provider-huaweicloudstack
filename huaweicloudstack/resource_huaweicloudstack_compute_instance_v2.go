...
func resourceComputeInstanceV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating HuaweiCloudStack server: %s", err)
	}
...
func resourceComputeInstanceV2Read(d *schema.ResourceData, meta interface{}) error {
	...
	// AccessIPv4/v6 isn't standard in HuaweiCloudStack, but there have been reports
	...
	if !ok {
		return fmt.Errorf("Error setting HuaweiCloudStack server's flavor: %v", server.Flavor)
	}
...
func resourceComputeInstanceV2Update(d *schema.ResourceData, meta interface{}) error {
	...
	if updateOpts != (servers.UpdateOpts{}) {
		...
		if err != nil {
			return fmt.Errorf("Error updating HuaweiCloudStack server: %s", err)
		}
	}
	...
	if d.HasChange("metadata") {
		...
		if err != nil {
			return fmt.Errorf("Error updating HuaweiCloudStack server (%s) metadata: %s", d.Id(), err)
		}
	}
	...
	if d.HasChange("security_groups") {
		...
		for _, g := range secgroupsToRemove.List() {
			...
			if err != nil && err.Error() != "EOF" {
				...
				return fmt.Errorf("Error removing security group (%s) from HuaweiCloudStack server (%s): %s", g, d.Id(), err)
			} else {
	...
	if d.HasChange("admin_pass") {
		if newPwd, ok := d.Get("admin_pass").(string); ok {
			...
			if err != nil {
				return fmt.Errorf("Error changing admin password of HuaweiCloudStack server (%s): %s", d.Id(), err)
			}
		}
	}
	...
	if d.HasChange("flavor_id") || d.HasChange("flavor_name") {
		...
		if err != nil {
			return fmt.Errorf("Error resizing HuaweiCloudStack server: %s", err)
		}
		...
		if err != nil {
			return fmt.Errorf("Error confirming resize of HuaweiCloudStack server: %s", err)
		}
...
func resourceComputeInstanceV2Delete(d *schema.ResourceData, meta interface{}) error {
	...
	if d.Get("stop_before_destroy").(bool) {
		...
		if err != nil {
			log.Printf("[WARN] Error stopping HuaweiCloudStack instance: %s", err)
		} else {
	...
	log.Printf("[DEBUG] Deleting HuaweiCloudStack Instance %s", d.Id())
	err = servers.Delete(computeClient, d.Id()).ExtractErr()
	if err != nil {
		return fmt.Errorf("Error deleting HuaweiCloudStack server: %s", err)
	}
...
func resourceComputeInstanceV2ImportState(d *schema.ResourceData, meta interface{}) ([]*schema.ResourceData, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Error reading compute instance %s: %s", d.Id(), err)
	}

	raw := servers.Get(computeClient, d.Id())
	if raw.Err != nil {
		return nil, CheckDeleted(d, raw.Err, "compute instance")
	}
	...
	log.Printf("[DEBUG] Retrieved compute instance %s volume attachments: %#v",
	...
	if len(serverWithAttachments.VolumesAttached) > 0 {
		...
		if err != nil {
			return nil, fmt.Errorf("Error creating HuaweiCloudStack volume client: %s", err)
		}
	...
	if v := d.Get("personality"); v != nil {
		...
		if len(personalityList) > 0 {
			for _, p := range personalityList {
				...
				log.Printf("[DEBUG] HuaweiCloudStack Compute Instance Personality: %+v", file)
