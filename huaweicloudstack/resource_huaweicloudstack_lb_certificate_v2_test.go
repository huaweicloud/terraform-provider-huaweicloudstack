		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLBV2CertificateExists("huaweicloudstack_lb_certificate_v2.certificate_1", &c),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"huaweicloudstack_lb_certificate_v2.certificate_1", "name", "certificate_1_updated"),
				),
			},
		},
	})
...
func testAccCheckLBV2CertificateDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "huaweicloudstack_lb_certificate_v2" {
			continue
		}
