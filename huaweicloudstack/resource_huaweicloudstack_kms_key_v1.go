...
func resourceKmsKeyV1Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating HuaweiCloudStack kms key client: %s", err)
	}
	...
	if err != nil {
		return fmt.Errorf("Error creating HuaweiCloudStack key: %s", err)
	}
...
func resourceKmsKeyV1Update(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("key_alias") {
		...
		if err != nil {
			return fmt.Errorf("Error updating HuaweiCloudStack key: %s", err)
		}
	}
