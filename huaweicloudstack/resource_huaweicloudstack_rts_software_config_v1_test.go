		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckRtsSoftwareConfigV1Exists("huaweicloudstack_rts_software_config_v1.config_1", &config),
					resource.TestCheckResourceAttr(
						"huaweicloudstack_rts_software_config_v1.config_1", "name", rtsName),
					resource.TestCheckResourceAttr(
						"huaweicloudstack_rts_software_config_v1.config_1", "group", "script"),
				),
			},
		},
	})
...
func testAccCheckRtsSoftwareConfigV1Destroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "huaweicloudstack_rts_software_config_v1" {
			continue
		}
...
func testAccRtsSoftwareConfigV1_basic(rtsName string) string {
	return fmt.Sprintf(`
resource "huaweicloudstack_rts_software_config_v1" "config_1" {
  name = "%s"
  output_values = [{
    type = "String"
    name = "result"
