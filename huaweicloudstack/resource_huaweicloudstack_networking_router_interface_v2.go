...
func resourceNetworkingRouterInterfaceV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating HuaweiCloudStack Neutron router interface: %s", err)
	}
...
func resourceNetworkingRouterInterfaceV2Read(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		...
		return fmt.Errorf("Error retrieving HuaweiCloudStack Neutron Router Interface: %s", err)
	}
...
func resourceNetworkingRouterInterfaceV2Delete(d *schema.ResourceData, meta interface{}) error {
	...
	_, err = stateConf.WaitForState()
	if err != nil {
		return fmt.Errorf("Error deleting HuaweiCloudStack Neutron Router Interface: %s", err)
	}
...
func waitForRouterInterfaceActive(networkingClient *golangsdk.ServiceClient, rId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		log.Printf("[DEBUG] HuaweiCloudStack Neutron Router Interface: %+v", r)
...
func waitForRouterInterfaceDelete(networkingClient *golangsdk.ServiceClient, d *schema.ResourceData) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		log.Printf("[DEBUG] Attempting to delete HuaweiCloudStack Router Interface %s.", routerInterfaceId)
		...
		if err != nil {
			if _, ok := err.(golangsdk.ErrDefault404); ok {
				log.Printf("[DEBUG] Successfully deleted HuaweiCloudStack Router Interface %s", routerInterfaceId)
		...
		log.Printf("[DEBUG] HuaweiCloudStack Router Interface %s is still active.", routerInterfaceId)
		return r, "ACTIVE", nil
	}
}
