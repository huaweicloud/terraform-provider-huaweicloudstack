...
func dataSourceImagesImageV2Read(d *schema.ResourceData, meta interface{}) error {
	...
	visibility := imagesImageV2VisibilityFromString(d.Get("visibility").(string))
...
func dataSourceImagesImageV2Attributes(d *schema.ResourceData, image *images.Image) error {
	log.Printf("[DEBUG] huaweicloudstack_images_image details: %#v", image)
...
func imagesImageV2VisibilityFromString(v string) images.ImageVisibility {
	switch v {
	case "public":
