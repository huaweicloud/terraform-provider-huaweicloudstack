		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLBV2PoolExists("huaweicloudstack_lb_pool_v2.pool_1", &pool),
				),
			},
			{
				Config: TestAccLBV2PoolConfig_update,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("huaweicloudstack_lb_pool_v2.pool_1", "name", "pool_1_updated"),
				),
			},
		},
	})
...
func testAccCheckLBV2PoolDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "huaweicloudstack_lb_pool_v2" {
			continue
		}
