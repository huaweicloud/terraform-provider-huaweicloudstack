		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckRtsStackV1Exists("huaweicloudstack_rts_stack_v1.stack_1", &stacks),
					resource.TestCheckResourceAttr(
						"huaweicloudstack_rts_stack_v1.stack_1", "name", "terra_stack"),
					resource.TestCheckResourceAttr(
						"huaweicloudstack_rts_stack_v1.stack_1", "status", "CREATE_COMPLETE"),
					resource.TestCheckResourceAttr(
						"huaweicloudstack_rts_stack_v1.stack_1", "disable_rollback", "true"),
					resource.TestCheckResourceAttr(
						"huaweicloudstack_rts_stack_v1.stack_1", "timeout_mins", "60"),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckRtsStackV1Exists("huaweicloudstack_rts_stack_v1.stack_1", &stacks),
					resource.TestCheckResourceAttr(
						"huaweicloudstack_rts_stack_v1.stack_1", "disable_rollback", "false"),
					resource.TestCheckResourceAttr(
						"huaweicloudstack_rts_stack_v1.stack_1", "timeout_mins", "50"),
					resource.TestCheckResourceAttr(
						"huaweicloudstack_rts_stack_v1.stack_1", "status", "UPDATE_COMPLETE"),
				),
			},
		},
	})
...
func testAccCheckRtsStackV1Destroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "huaweicloudstack_rts_stack_v1" {
			continue
		}
