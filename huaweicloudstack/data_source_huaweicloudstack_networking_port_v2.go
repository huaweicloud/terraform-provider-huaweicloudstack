...
func dataSourceNetworkingPortV2Read(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Unable to list huaweicloudstack_networking_ports_v2: %s", err)
	}
	...
	if err != nil {
		return fmt.Errorf("Unable to retrieve huaweicloudstack_networking_ports_v2: %s", err)
	}

	if len(allPorts) == 0 {
		return fmt.Errorf("No huaweicloudstack_networking_port_v2 found")
	}
	...
	if v, ok := d.GetOk("fixed_ip"); ok {
		...
		if len(portsList) == 0 {
			log.Printf("No huaweicloudstack_networking_port_v2 found after the 'fixed_ip' filter")
			return fmt.Errorf("No huaweicloudstack_networking_port_v2 found")
		}
	} else {
		portsList = allPorts
	...
	if len(securityGroups) > 0 {
		...
		if len(sgPorts) == 0 {
			log.Printf("[DEBUG] No huaweicloudstack_networking_port_v2 found after the 'security_group_ids' filter")
			return fmt.Errorf("No huaweicloudstack_networking_port_v2 found")
		}
		portsList = sgPorts
	}
	...
	if len(portsList) > 1 {
		return fmt.Errorf("More than one huaweicloudstack_networking_port_v2 found (%d)", len(portsList))
	}

	port := portsList[0]

	log.Printf("[DEBUG] Retrieved huaweicloudstack_networking_port_v2 %s: %+v", port.ID, port)
	d.SetId(port.ID)
