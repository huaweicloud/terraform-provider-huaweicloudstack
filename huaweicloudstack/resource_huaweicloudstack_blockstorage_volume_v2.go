...
func resourceBlockStorageVolumeV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating HuaweiCloudStack volume: %s", err)
	}
...
func resourceBlockStorageVolumeV2Update(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("size") {
		...
		if err != nil {
			return fmt.Errorf("Error extending huaweicloudstack_blockstorage_volume_v2 %s size: %s", d.Id(), err)
		}
	...
	if err != nil {
		return fmt.Errorf("Error updating HuaweiCloudStack volume: %s", err)
	}
